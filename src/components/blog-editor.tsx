      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-4">
            {(isSaving || isAnalyzing) && (
              <div className="p-4 border border-primary/50 bg-primary/5 rounded-md flex flex-col items-center justify-center gap-2 text-center">
                ...
                <p className="text-sm font-medium text-primary">{isSaving ? "Saving your draft..." : "Submitting article for review..."}</p>
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent>
              <div className="flex items-center justify-between">
                ...
                <Badge variant={status === "PUBLISHED" ? "success" : status === "DRAFT" ? "warning" : "secondary"}>{status}</Badge>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <Button className="w-full" onClick={handleRunAiEvaluation} disabled={isEvaluating || aiAttempts >= 3 || status !== "DRAFT"}>
                {isEvaluating ? <Spinner className="mr-2 h-4 w-4" /> : <Wand2 className="mr-2 h-4 w-4" />}
                Run AI Evaluation
              </Button>
              {aiAttempts >= 3 && <p className="text-xs text-destructive mt-3 text-center">Maximum attempts reached.</p>}
              {!draftId && status === "DRAFT" && <p className="text-xs text-muted-foreground mt-3 text-center">Save draft to enable evaluation.</p>}
            </CardContent>
          </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <ul className="list-disc pl-5 text-sm space-y-1 text-muted-foreground">
                    {aiAnalysis.issues.map((s: string, i: number) => (
                      <li key={i}>{s}</li>
                    ))}
                  </ul>
                </div>
